...
class PlaceHolderDSLProcessor(BasicDSLProcessor):
    ...
    def process(self, code: str, input: dict, outputNames: List[str], preferredVisualReturnType: str,config:dict) -> ProgramOutput:
        ...
        program = None
        try:
            program = self.programDirectory.getProgram(itomhash)
            # Load and refresh the program directory
            programExecutor = self.programDirectory.getProgramExecutor()
            programOutput = programExecutor.executeProgram(program.name, ProgramInput(startTimestamp=0, inputs=innerInput), preferredVisualReturnType="html", config={})
        except:
        ...
        result = super().process(code, itomidstring, outputNames, "md",config)
